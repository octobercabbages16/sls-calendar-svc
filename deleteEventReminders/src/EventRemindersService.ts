...
export class EventRemindersService {
  ...
  async processRequest(event: any) {
    ...
    try {
      const deletedCount = await db('portal.event_reminders')
        .where({ id: reminderId })
        .del();
      ...
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: 'Event reminder deleted successfully' }),
      };
    } catch (error) {
      ...
      return {
        statusCode: 500,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ error: 'Failed to delete event reminder' }),
      };
    } finally {
      await db.destroy();
    }
