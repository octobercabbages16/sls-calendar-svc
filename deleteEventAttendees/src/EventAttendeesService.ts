...
export class EventAttendeesService {
  ...
  async processRequest(event: any) {
    ...
    try {
      const deletedCount = await db('portal.event_attendees')
        .where({ id: attendeeId })
        .del();
      ...
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: 'Event attendee deleted successfully' }),
      };
    } catch (error) {
      ...
      return {
        statusCode: 500,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ error: 'Failed to delete event attendee' }),
      };
    } finally {
      await db.destroy();
    }
